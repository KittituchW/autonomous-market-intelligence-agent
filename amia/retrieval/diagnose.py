"""Diagnostic: count docs per source in Qdrant vs on disk.

If Qdrant has 0 stocktwits docs but disk has 70+, the index is stale.
Fix: rerun `python -m amia.retrieval.index` to rebuild.
"""
import json
import os
...
def main() -> None:
    ...
    if qdrant_st == 0 and disk_st > 0:
        print("\n>>> Index is stale. Run `python -m amia.retrieval.index` to rebuild. <<<")
    elif qdrant_st < disk_st * 0.5:
        print("\n>>> Index is partial. Run `python -m amia.retrieval.index` to rebuild. <<<")
    else:
